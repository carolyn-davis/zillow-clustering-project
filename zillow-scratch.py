...
col_list = ['propertylandusetypeid', 'id', 'transactiondate',
            'id.1', 'propertycountylandusecode', 'rawcensustractandblock',
            'assessmentyear', 'roomcnt']
...
zillow_df = zillow_df.drop(columns='propertylandusedesc')
...
def new_index(df):
    ...
    df = df.rename(columns={'parcelid': 'parcel_id', 'logerror': 'target', 
                            ...
                            'yearbuilt': 'year_built', 'taxvaluedollarcnt': 'tax_dollar_value', 
...
#Decided not to include acreage as a feature because there are too many null values 
#imputing wasn't a move I could make in confidence at that time 
